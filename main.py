...
def screenshot():
    ...
    with Image.open(io.BytesIO(r.stdout)) as pil_img:
...
def tap_area(x1, y1, x2, y2):
    ...
    time.sleep(random.uniform(0.23, 0.4) + random.uniform(-0.17, 0.17))
...
def takeoff_branch(log):
    ...
    for step in range(10):
        ...
        if any(k in btn for k in ["升级合约","升级"]) and "奖励" not in btn:
            ...
            tap_area(820,670,970,705); time.sleep(0.5); last_btn=""; repeat=0
            ...
            click_yellow_btn(); time.sleep(0.5)
            tap_area(618, 656, 905, 705); time.sleep(0.5); last_btn=""; repeat=0
        elif any(k in btn for k in ["奖励"]):
            log("    点击领取奖励按钮")
            click_yellow_btn(); time.sleep(0.33)
            for _ in range(5):
                log("    点击奖励弹窗（领取/升级）")
                tap_area(820,670,970,705); time.sleep(0.5)
                img3 = screenshot()
                # 检测"领取奖励并延期"按钮
                ...
                if any(k in popup_text for k in ["领取奖励并延期","领取","延期"]):
                    log("    检测到领取奖励并延期，点击")
                    tap_area(618, 656, 905, 705); time.sleep(0.5)
                    img3 = screenshot()
        ...
        time.sleep(random.uniform(0.33, 0.67))
    ...
    for step in range(8):
        ...
        if any(k in btn for k in ["延长合约","延长","延期"]):
            ...
            if np.mean(cv2.cvtColor(popup,cv2.COLOR_BGR2GRAY)) < 55:
                log("    点击选择停机位按钮"); click_yellow_btn(); time.sleep(0.4); img = screenshot()
            stand = find_available_stand(img)
            if stand:
                log(f"    选择空置机位 {stand}")
                tap_area(stand[0]-30,stand[1]-20,stand[0]+30,stand[1]+20)
                time.sleep(0.27); log("    点击确认"); click_yellow_btn(); time.sleep(0.33)
            else:
                log("    无空置机位，跳过此飞机")
...
def ground_branch(log):
    ...
    if np.mean(cv2.cvtColor(plus,cv2.COLOR_BGR2GRAY)) < 60:
        ...
        adb_run(["shell","input","keyevent","4"]); return False
    green_found = False
    for i in range(75):  # 最多75次点击（原15次×每5次检查一次）
        x = random.randint(960, 980) + random.randint(-2, 2)
        y = random.randint(680, 700) + random.randint(-2, 2)
        adb_run(["shell", "input", "tap", str(x), str(y)])
        time.sleep(0.1)
        # 每5次点击截图检查一次
        if (i + 1) % 5 == 0:
            img2 = screenshot()
            if is_green_at(img2, 900, 695):
                log(f"    滑块到位（{i+1}次点击）")
                green_found = True; break
            plus = img2[670:705,955:985]
            if np.mean(cv2.cvtColor(plus,cv2.COLOR_BGR2GRAY)) < 60:
                log("    加号不可用，人数不足，跳过")
                adb_run(["shell","input","keyevent","4"]); return False
    ...
    log("    点击机坪代理开关"); tap_area(940,773,956,787); time.sleep(0.27)
    img3 = screenshot()
...
def bot_loop(gui):
    ...
    while gui.running:
        ...
        if not found_any:
            ...
            maybe_idle(gui.log)
        time.sleep(random.uniform(0.5, 0.83))
...
class WoaGUI:
    ...
    def _build_ui(self):
        ...
        tk.Label(cf, text="地勤次数", font=("Helvetica",8), bg=PANEL, fg=GRAY).pack(pady=(6,0))
